<template>
  <div class="fruit-list-container">
    ...
    <div class="settle-box">
      ...
      <span>总价：{{ total }}</span>
      <!-- TODO: 3. 动态计算按钮的禁用状态 -->
      <button type="button" class="btn btn-primary" :disabled=disabled>结算</button>
    </div>
  </div>
</template>
...
const count = computed(() => {
  ...
  //   let item = fruitList[i]
  //   if (item.state) {
  //     num++;
  //   }
  // }
  // return num;
  return fruitList.reduce((prev, cur)=>prev + cur.state, 0)
})
...
const total = computed(()=>{
  ...
  //   let item = fruitList[i]
  //   if (item.state) {
  //     total = total + item.price * item.count * item.state
  //   }
  // }
  // return total;
  return fruitList.reduce((prev, cur)=>prev + cur.state * cur.price * cur.count, 0)
})
